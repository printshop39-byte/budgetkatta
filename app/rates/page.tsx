  ...
  return (
    <PageShell titleKey="rates.title" subtitleKey="rates.subtitle">
      <div className="space-y-10">
        ...
        <section>
          <div className="mb-3 flex items-center justify-between">
            <h2 className="font-display text-xl font-bold text-slate-200">{t('rates.market_title')}</h2>
            <span className="rounded-full border border-slate-800 bg-slate-900/[0.035] px-2.5 py-0.5 text-xs text-slate-400 font-deva">
            ...
            {marketCards.map((c) => (
              <div key={c.title[language]} className="glass-card p-5">
                ...
                <ul className="space-y-1.5">
                  {c.lines.map((line, i) => (
                    <li key={i} className="text-xs leading-relaxed text-slate-400 font-deva">
                      • {line[language]}
                    </li>
                  ))}
                </ul>
