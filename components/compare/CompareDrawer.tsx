          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="rounded-lg border border-white/10 px-3 py-1.5 text-sm text-white/50 transition-colors hover:border-white/30 hover:text-white font-deva"
              >
                {t('btn.clear')}
              </button>
              <button
            ...
            <table className="w-full min-w-[500px]">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="py-2 text-left text-sm font-normal text-white/50">Field</th>
                  {items.map((item, i) => (
                    <th
                      key={i}
              ...
              <tbody>
                {cols.map((col, ci) => (
                  <tr key={col} className="border-b border-white/5">
                    <td className="py-2.5 pr-4 text-sm text-white/60">{col}</td>
                    {items.map((item, ii) => (
                      <td
                        key={ii}
                        className="py-2.5 text-center text-sm text-white font-deva"
                      >
                        {item.data?.[ci] ?? '—'}
                      </td>
                    ))}
